...
## Acceptance Criteria
- [ ] Given an extracted clip (audio file) and its metadata (Key, BPM,
      timestamp, session ID), the clip is uploaded to Fulcra via the
      Python SDK's `upload_file`.
- [ ] A corresponding record (e.g. a `MusicalIdea` annotation/data type) is
...
- [ ] The published record can be retrieved/queried afterward via the SDK
