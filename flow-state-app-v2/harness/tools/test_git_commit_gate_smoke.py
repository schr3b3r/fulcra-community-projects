...
This is the most safety-critical smoke test we have — it proves the gate
that's supposed to prevent regressions actually works, both when it should
allow a commit and when it should refuse one.
"""
...
def main():
    ...
    assert not tests_dir.exists(), (
        ...
        "smoke test on a repo state with no existing test suite."
    )
