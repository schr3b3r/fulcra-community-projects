...
Security posture (read this before extending the allowed scope):
  ...
  - PATH is pointed at the harness's own .venv so `python`/`pip` resolve to
    an environment that already has the app's typical dependencies
    (fastapi, uvicorn, etc.) available, without the agent needing to create
    or manage its own virtual environment.
...
What this tool does NOT do, and should not be assumed to do:
  - It does NOT prevent a malicious or buggy command from affecting things
    ...
    limitation appropriate for a learning project under direct human
    supervision — NOT something to hand to an untrusted agent unsupervised.
"""
...
RUN_COMMAND_SCHEMA = {
    ...
    "description": (
        ...
        "fastapi/uvicorn already installed. Has a hard timeout — do NOT "
        "use this to start long-running foreground servers, only for "
        "short one-shot verification commands."
    ),
    "parameters": {
        "type": "object",
        "properties": {
