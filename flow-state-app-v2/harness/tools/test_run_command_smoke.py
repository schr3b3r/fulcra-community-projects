...
def main():
    ...
    print(output)
    assert "app" in output  # cwd should be .../flow-state-app-v2/app

    print("\n--- Test 3: python resolves to the harness's own venv ---")
    output = run_command("python -c \"import fastapi; print('fastapi OK')\"")
    print(output)
    assert "fastapi OK" in output, (
        "Expected python to resolve to the harness venv with fastapi "
        "installed (see _HARNESS_VENV_BIN in run_command.py)"
    )
