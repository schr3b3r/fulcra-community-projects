...
single sandbox directory (the `app/` folder where the recreated Flow State
app lives). The agent cannot use these tools to read or write anything
outside that boundary, including the harness's own code, its `.env` secrets,
or the rest of the git repo.
...
#   harness/tools/filesystem.py -> flow-state-app-v2/ -> app/
...
def _resolve_in_sandbox(relative_path: str) -> Path:
    ...
    # is_relative_to requires Python 3.9+; we're on 3.11+ per pyproject.toml.
