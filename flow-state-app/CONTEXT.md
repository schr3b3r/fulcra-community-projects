...
1. **WebSockets over Browser Buffering:** Browsers drop standard `MediaRecorder` buffers if they crash. We chunk audio every 2 seconds via WebSockets to FastAPI so the jam is safely streamed to disk in real-time. (Note: this auto-upgrades to `wss://` if the app is placed behind an HTTPS tunnel).
2. **FastAPI over 3rd-Party Bots:** We abandoned Discord/Telegram bots because they introduce heavy C++ dependencies or end-to-end encryption blockers for raw audio streaming. The browser `getUserMedia` gives us pristine audio (with echo cancellation disabled and autoGain on).
3. **Semantic Querying over File Scanning:** The frontend Review Feed does *not* scan directories for WAV files. It uses `fulcra-api catalog` to dynamically look up the user's `MusicalIdea` ID, and queries it. Fulcra acts as the database, making the UI instantly queryable without a traditional SQL backend.
