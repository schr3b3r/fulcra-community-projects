...
- GitHub API access: use `requests` (or Python's stdlib http.client) directly against GitHub's REST/GraphQL APIs -- do NOT shell out to the `gh` CLI, since the skill must work with an arbitrary provided token/identity, not whatever `gh` happens to be logged in as on the host machine.
- Markdown generation: plain string formatting / an f-string-based template is sufficient for a single well-formatted document; no need for a heavier templating engine.
